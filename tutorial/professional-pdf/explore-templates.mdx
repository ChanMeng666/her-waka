...
description: "Discover 7 professional document templates from Typst Universe — from invoices to reports — with ready-to-copy prompts for Gemini CLI."
---

Now that you know the workflow — initialise a template, customise with AI, compile, review — let's explore what else you can create. Each template below comes from [Typst Universe](https://typst.app/universe), Typst's free community template library.
...
Pick any template below. Run the `typst init` command to download it, then use the Gemini CLI prompts to customise it for your needs.
...
<AccordionGroup>
  <Accordion title="1. Cover Letter — Advanced (Simplest)">
    **Use case:** Take your cover letter further with multi-page or styled variants.
    ...
    ```bash title="Initialise the template"
...
```text title="Copy this prompt — Create from scratch"
